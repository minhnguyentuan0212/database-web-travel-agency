...
BEGIN
    ...
    ELSE
        ...
        THEN 
			SELECT SUM(CASE WHEN DATEDIFF(NEW.NgayDangKy, KhachHang.NgaySinh) / 365 <= 10 THEN Tour.GiaVeDoanTreEm ELSE Tour.GiaVeDoanNguoiLon END)
			INTO total_price
			FROM KhachDoan
		...
		ELSE
			SELECT SUM(CASE WHEN DATEDIFF(NEW.NgayDangKy, KhachHang.NgaySinh) / 365 <= 10 THEN Tour.GiaVeLeTreEm ELSE Tour.GiaVeLeNguoiLon END)
			INTO total_price
			FROM KhachDoan
			JOIN KhachDoan_Gom_KhachLe ON KhachDoan.MaKD = KhachDoan_Gom_KhachLe.MaKD
			JOIN KhachHang ON KhachHang.MaKH = KhachDoan_Gom_KhachLe.MaKH
